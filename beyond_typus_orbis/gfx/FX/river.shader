...
PixelShader = 
{
	...
	[[
		float4 main( VS_OUTPUT In ) : PDX_COLOR
		{
			...
			vBottom = GetOverlay( vBottom, ColorMap, 0.5f ) * 1.9; //added *2
			...
			float3 vColor = lerp( vBottom, vWaterSurface.xyz, vWaterSurface.a * 0.0f ); // changed from 0.8 to 0.2
			...
			return float4( 
				ComposeSpecular( 
					vColor, 
					specular * ( 1.0f - In.vPrePos_Fade.w ) * vWaterSurface.a * vFoW 
				), 
				vBottomAlpha * ( 1.0f - In.vPrePos_Fade.w ) * (1.0f - TI ) 
			) * fShadowTerm;
		}
	]]
