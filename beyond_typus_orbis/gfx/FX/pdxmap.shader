	...
	[[
		...
		float4 main( VS_OUTPUT_TERRAIN Input ) : PDX_COLOR
		{
		...
		#endif
			{
				...
				float colorRange = testCam > 1.0f? 1.0f: testCam;		
				
				//parameters for map look adjustment
				float blendRatio = 0.45f; // map look at the max zoom level
										  // from 0.0f (pure terrain) to 1.0f (pure color)
				
				float detailOpacity = 0.7f; // province detail opacity at the min zoom level
											// from 0.5f (max detail opacity) to 1.0f (no changes from the max zoom)
											
				float brightnessMultiplier = 1.25f; // general map brightness
			
				float adjustedTerrainColorValue = max(detailOpacity, length(vTerrainDiffuseSample.rgb)*1.25);

				float3 vEnd = lerp(vTerrainDiffuseSample.rgb, vColorMapSample.rgb, blendRatio);
				...
				vOut = CalculateMapLighting( vOut, vHeightNormalSample )*brightnessMultiplier; //regulates brightness of colored mapmodes on all zoom stages
